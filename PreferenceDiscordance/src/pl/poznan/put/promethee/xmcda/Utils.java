...
public class Utils {
	...
	/**
	 * Parses the command-line and search for the input directory (options
	 * {@code -i} or {@code --input-directory}) and for the output directory
	 * (options {@code -o} or {@code --output-directory}).
	 *
	 * @param args
	 *            the arguments of the command-line (its length must be equal to
	 *            4).
	 * @return an {@link Arguments argument} with non-null fields
	 *         {@code inputDirectory} and {@code outputDirectory}
	 * @throws InvalidCommandLineException
	 *             in one or both input/output directories are not present in
	 *             the command line.
	 */
	...
	public static void loadXMCDAv3(XMCDA xmcda, final File file, boolean mandatory,
		...
		if (!file.exists()) {
			if (mandatory) {
				...
				// x_execution_results.addInfo("Optional file %s absent" %
				// basefilename)
				return;
		}
	...
	public static void readXMCDAv2_and_update(org.xmcda.v2_2_1.XMCDA xmcda_v2, File file,
			String[] load_tags) throws FileNotFoundException, JAXBException, SAXException {
		final org.xmcda.v2_2_1.XMCDA new_xmcda = XMCDAParser.readXMCDA(file, load_tags);
	...
	/**
	 * Simply returns {@link Throwable#getMessage() throwable.getMessage()}, or
	 * {@code "unknown"} if it is @{code null}.
	 *
	 * @param throwable
	 *            a non-null {@link Throwable}
	...
	/**
	 * Writes the XMCDA file containing the information provided to build the
	 * XMCDA tag "{@code programExecutionResult}" in XMCDA v3, or
	 * "{@code methodMessages}" in XMCDA v2.x.
	 *
	 * @param prgExecResultsFile
	 *            the file to write
	 * @param errors
	 *            a {@link ProgramExecutionResult} object
	 * @param xmcdaVersion
	 *            indicates which {@link XMCDA_VERSION} to use when writing the
	 *            file
	 */
	public static void writeProgramExecutionResults(File prgExecResultsFile, ProgramExecutionResult errors,
			XMCDA_VERSION xmcdaVersion) throws Throwable {
		...
		case v2:
			...
			break;
		default:
			// in case the enum has some more values in the future and the new
			// ones have not been added here
	...
	/**
	 * Calls
	 * {@link #writeProgramExecutionResults(File, ProgramExecutionResult, XMCDA_VERSION)}
	 * then exits. The return status code is {@link Status#exitStatus()
	 * errors.getStatus().getExitStatus}, or {@link Status#ERROR}'s
	 * {@code exitStatus()} in case of failure when writing the execution
	 * results.
	 *
	 * @param prgExecResultsFile
	 * @param errors
	...
	public static void writeProgramExecutionResultsAndExit(File prgExecResultsFile, ProgramExecutionResult errors,
		...
		try {
			...
			// We choose here not to clean up the file, in case some valuable
			// information were successfully written
