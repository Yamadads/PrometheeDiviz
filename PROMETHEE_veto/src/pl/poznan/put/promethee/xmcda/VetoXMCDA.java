...
import pl.poznan.put.promethee.veto.Veto;
...
public class VetoXMCDA {
	...
	private static Map<String, InputFile> initFiles() {
		...
		files.put("alternatives", new InputFile("alternatives", "alternatives", "alternatives.xml", "alternatives.xml", true));
		files.put("categoriesProfiles",
				new InputFile("categoriesProfiles", "categoriesProfiles", "categories_profiles.xml", "categories_profiles.xml", false));
		files.put("criteria", new InputFile("criteria", "criteria", "criteria.xml", "criteria.xml", true));
		files.put("criteriaScales", new InputFile("criteriaScales", "criteriaScales", "criteria.xml", "criteria_scales.xml", true));
		files.put("performanceTable",
				new InputFile("performanceTable", "performanceTable", "performance_table.xml", "performance_table.xml", true));
		files.put("criteriaWeights", new InputFile("criteriaValues", "criteriaValues", "weights.xml", "weights.xml", false));
	...
	private static Map<String, Map<String, Double>> calcResults(InputsHandler.Inputs inputs,
		...
		try {
			results = Veto.calcTotalVeto(inputs);
		} catch (Throwable t) {
	...
	private static Map<String, Map<String, Map<String, Double>>> calcPartialResults(InputsHandler.Inputs inputs,
		...
		try {
			results = Veto.calcPartialVeto(inputs);
		} catch (Throwable t) {
			executionResult.addError(Utils.getMessage("The calculation could not be performed, reason: ", t));
			return results;
		}
